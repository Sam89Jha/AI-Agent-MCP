...
# Get agent details from agent-info.txt file
if [ -f "agent-info.txt" ]; then
    AGENT_ID=$(grep "Agent ID:" agent-info.txt | cut -d' ' -f3)
    ALIAS_ID=$(grep "Alias ID:" agent-info.txt | cut -d' ' -f3)
else
    echo "❌ agent-info.txt not found. Please run deploy.sh first."
    exit 1
fi

if [ -z "$AGENT_ID" ] || [ -z "$ALIAS_ID" ]; then
    echo "❌ Agent ID or Alias ID not found in agent-info.txt. Please run deploy.sh first."
    exit 1
fi
